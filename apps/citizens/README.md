...
- **`notify_officers_of_new_registration`** — on every new registration (`created=True`), notifies every `OFFICER` whose `ward` matches the citizen's ward, plus every `ADMIN`, via a web `Notification` deep-linked to the citizen's detail page (`Notification.related_citizen`). The matching ward officer(s) also get a real SMS via `send_sms()` to their own `User.phone_number` — admins get the web notification only, not SMS, to avoid mass-texting every admin account on every single registration.
